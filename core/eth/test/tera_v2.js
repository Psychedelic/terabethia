...
describe("Terabethia", function () {
  it("Should return the new greeting once it's changed", async function () {
    ...
    const TerabethiaV2 = await ethers.getContractFactory("TerabethiaV2");
    ...
    const impl = await TerabethiaV2.deploy();
    await impl.deployed();

    // const tera = await Proxy.deploy(300);
    const tera = await upgrades.deployProxy(TerabethiaV2, [STARKNET_CONTRACT]);
    await tera.deployed();
